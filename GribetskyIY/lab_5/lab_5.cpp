...
int result [n][n];//результат действия
...
void summ_or_razn (int mat_1[n][n], int mat_2[n][n], int itog [n][n],int oper){//сумма и разность
    if((n_1==n_2)&&(m_1==m_2))
    {
        if(oper==1){
            for(int i = 0; i<n_1 ; i++){
...
{
    char ch;
    int operation;//действие над матрицами
    do{
    correct = true;
    ...
    {
        case 1://сложение
            summ_or_razn(matr_1, matr_2, result, operation);
            break;
        case 2://вычитание
            summ_or_razn(matr_1, matr_2, result, operation);
            break;
        case 3://умножение
            proizved(matr_1, matr_2, result);
            break;
